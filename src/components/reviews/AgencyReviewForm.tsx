...
export function AgencyReviewForm({ agencyId, onSuccess, onCancel }: AgencyReviewFormProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const { error: submitError } = await supabase.from('agency_reviews').insert([
        {
          agency_id: agencyId,
          user_id: user.id,
          rating,
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <textarea
          ...
          placeholder="Compartilhe sua experiência com esta agência..."
          required
        />
      </div>
